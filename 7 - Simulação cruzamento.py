...
titulo = pygame.display.set_caption("Simulação 7")
c_rua,l_rua =  c , l/2.8
...
class Carro:
    ...
    def mostrar_velocidade(self, tela):
        ...
        texto = fonte.render(velocidade_texto, True, (255, 255, 255))
                ...
                if self.velocidade_x < 0:  # Via1 (indo para esquerda)
                    ...
                    for carro in carros_via1:
                        if self != carro :
                            ...
                            if self.x > carro.x and dx < c_carro*1.2 and faixa_branca1 <= self.x < c:
                                parado_semaforo = True
                else: # Via1 (indo para direita)
                    if faixa_branca2 - c_carro/2 <= self.x+c_carro < faixa_branca2:
                        parado_semaforo = True
            ...
            if self.velocidade_y > 0:  # Via3 (indo para baixo)
                ...
                for carro in carros_via3:
                    if self != carro :
                        ...
                        if self.y < carro.y and dy < c_carro*1.2 and 0 <= self.y+c_carro < faixa_branca3:
                            parado_semaforo = True
            else:  # Via4 (indo para cima)
                if faixa_branca4 <= self.y < faixa_branca4 + c_carro/4:
                    parado_semaforo = True
...
# CRIAÇÃO CARROS:
    ...
    if tempo_atual3 - ultimo_tempo3 > tempo_proximo_carro3:
        ...
        if len(carros_via3)<3 and estado2 == 'vermelho':
            dy = 0 
            for carro in carros_via3 :
    ...
    if tempo_atual4 - ultimo_tempo4 > tempo_proximo_carro4:
        ...
        if len(carros_via4)<3 and estado2 == 'vermelho':
            dy = 0 
            for carro in carros_via4 :
                if carro.velocidade_y < 0:
...
# Legenda comandos:
    ...
    texto_colisao = fonte.render(texto_contador, True, (255, 255, 255))
    #legenda semáforos
    mudar_semaforo1 = "esquerdo mouse troca o sinal -->"
    texto_semaforo1 = fonte.render(mudar_semaforo1,True,(255,255,255))
    mudar_semaforo2 = "<-- direito do mouse troca o sinal"
    texto_semaforo2 = fonte.render(mudar_semaforo2,True,(255,255,255))
    # legenda comandos limpar
    limpar_via1 = "aperte '1' para limpar via 1"
    texto_limpar1 = fonte.render(limpar_via1,True,(255,255,255))
    limpar_via2 = "aperte '2' para limpar via 2"
    texto_limpar2 = fonte.render(limpar_via2,True,(255,255,255))
    limpar_via3 = "aperte '3' para limpar via 3"
    texto_limpar3 = fonte.render(limpar_via3,True,(255,255,255))
    limpar_via4 = "aperte '4' para limpar via 4"
    texto_limpar4 = fonte.render(limpar_via4,True,(255,255,255))
    limpar_tudo = "aperte 'c' para limpar tudo"
    texto_limpar_tudo = fonte.render(limpar_tudo,True,(255,255,255))
...
# Fim da Simulação
print("\nVocê rodou a Simulação 7 :")
print("A simulação durou %.2f segundos" %(tempo_decorrido))
print("Houve",contador_colisoes,"colisões na simulação\n")
pygame.quit()