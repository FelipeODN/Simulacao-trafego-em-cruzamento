...
titulo = pygame.display.set_caption("Simulação 10")
c_rua,l_rua =  c , l/2.8
...
class Carro:
    ...
    def mudar_via(self):
        ...
        if self in carros_via1 and self.virar == 'via4':
            if faixa_branca4_x+10 <= self.x <= faixa_branca4_x+70:
                novo_carro = Carro(self.cor,faixa_branca4_x+l_rua/4,self.y,45,90,0,self.velocidade_x,90,None)
                carros_via4.append(novo_carro)
                carros_via1.remove(self)
                return novo_carro
        if self in carros_via1 and self.virar == 'via3':
            if faixa_branca3_x+10 <= self.x+c_carro <= faixa_branca3_x+80:
                novo_carro = Carro(self.cor,faixa_branca3_x+l_rua/4,self.y,45,90,0,-self.velocidade_x,-90,None)
                carros_via3.append(novo_carro)
                carros_via1.remove(self)
                return novo_carro
        '''# mudança de via para carros na via2
        if self in carros_via2 and self.virar == 'via':
            if faixa_branca4_x+10 <= self.x <= faixa_branca4_x+70:
                novo_carro = Carro(self.cor,faixa_branca4_x+l_rua/4,self.y,45,90,0,self.velocidade_x,90,None)
                carros_via4.append(novo_carro)
                carros_via1.remove(self)
...
data1 = {'PA': [1/2, 1],         # Probabilidades acumuladas
        'Min': ['via3', 'via1'],  # Opções mínimas
        'Max': ['via4', 'via1']}  # Opções máximas
df1 = pd.DataFrame(data1)
PA = df1["PA"]

# definir virar ou não.
def mudar_via():
    x = np.random.rand()  # Gera um número aleatório entre 0 e 1
    for i, pa in enumerate(df1['PA']):  # i = índice, pa = valor da probabilidade
        if x <= pa:
            via_escolhida = random.choice([df1['Min'][i], df1['Max'][i]])
            return via_escolhida
